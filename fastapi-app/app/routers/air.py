...
from app.schemas.air import AirHistoryReport, AirHistoryReportWithTotal, AirRealtimeReport
...
def air_router(product_line_db_list: List[Dict[str, Any]], socketClient: Dict[tuple, SocketClient]) -> APIRouter:
    router = APIRouter()
    air_crud = AirCRUD()
    ...
    async def _get_db(product: str, line:str) ->AsyncSession:
        ...
        db_user, db_pass, db_server, db_port, db_name, valve_id_list = _get_connection_info(product, line)
    ...
    @router.get("/realtime", response_model=AirRealtimeReport, dependencies=[Depends(api_key_auth)])
    async def get_realtime_report_values(product: str, line: str, padding_secs: int = 0, latest_mins: int = 1):
        ...
        try:
            ...
            x_axis, y_axis, race_data = air_crud.get_air_realtime(socketClient[(product, line)], machine_info_dict, padding_secs, latest_mins)
            er = AirRealtimeReport(
                line = AirHistoryReport(x_axis=x_axis, y_axis=y_axis, mc_map=machine_name_dict),
                race = race_data
            ) 
            return er
        except Exception as e:
    ...
    @router.get("/daily", response_model=AirHistoryReportWithTotal, dependencies=[Depends(api_key_auth)])
    async def get_daily_report_values(product: str, line: str, year: int, month: int, date: int):
        ...
        try:
            ...
            x_axis, y_axis, total = await air_crud.get_air_consumption_daily(machine_info_dict, date, month, year, db)
            er = AirHistoryReportWithTotal(x_axis=x_axis, y_axis=y_axis, total=total, mc_map=machine_name_dict)
            await db.close()
            return er
    ...
    @router.get("/monthly", response_model=AirHistoryReportWithTotal, dependencies=[Depends(api_key_auth)])
    async def get_monthly_report_values(product: str, line: str, year: int, month: int):
        ...
        try:
            db, machine_info_dict, machine_name_dict = await _get_db(product, line)
            x_axis, y_axis, total = await air_crud.get_air_consumption_monthly(machine_info_dict, month, year, db)
            er = AirHistoryReportWithTotal(x_axis=x_axis, y_axis=y_axis, total=total, mc_map=machine_name_dict)
            await db.close()
            return er
        except InvalidMonthValue:
    ...
    @router.get("/yearly", response_model=AirHistoryReportWithTotal, dependencies=[Depends(api_key_auth)])
    async def get_yearly_report_values(product: str, line: str, year: int):
        ...
        try:
            db, machine_info_dict, machine_name_dict = await _get_db(product, line)
            x_axis, y_axis, total = await air_crud.get_air_consumption_yearly(machine_info_dict, year, db)
            er = AirHistoryReportWithTotal(x_axis=x_axis, y_axis=y_axis, total=total, mc_map=machine_name_dict)
            await db.close()
            return er
        except InvalidYearValue:

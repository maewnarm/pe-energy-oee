...
class EnergyCRUD:
    ...
    async def get_energy_consumption_daily(
        ...
        assert isinstance(
            machine_info_dict, dict
        ), f"[get_energy_consumption_daily] machine_info_dict is not a dictionary"
        assert isinstance(date, int), f"[get_energy_consumption_daily] date is not int"
        assert isinstance(
            month, int
        ), f"[get_energy_consumption_daily] month is not int"
        assert isinstance(year, int), f"[get_energy_consumption_daily] year is not int"
        assert isinstance(
            db, AsyncSession
        ), f"[get_energy_consumption_daily] year is not AsyncSession"
                    ...
                    if (x, mc_id) in energy_data:
                        ...
                        for delta in range(1, keep_interval_secs + 1):
                            ...
                            if (keep_time_str, mc_id) in energy_data:
                                y_axis[idx][mc_name] = energy_data[
                                    (keep_time_str, mc_id)
                                ]
                                break
